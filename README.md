...
Then I thought Go's concurrent/parallel and synchronous mecanism is suitable model for the present stage of Swift(If you want to write Server on the MultiCore Machine). Because we can easy to make async operations wituhout callback chains, can use full cores with simple syntax and easy to share the memory via Channel between a Thread and a Thread.
